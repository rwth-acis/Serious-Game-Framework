module.exports = function (router, options, dbProvider) {
    ...
    router.post(collectorRoot + 'badges', function (req, res) {
     ...
     router.post(collectorRoot + 'gamesdesign', function (req, res) {
        var gamesdata = JSON.parse(req.headers.detailsjson).gamesdata;
        var email = "";
        var gameId = "";
        var gameName = "";
        for(var i= 0; i < gamesdata.length; i++){
            email = gamesdata[i].oidcEmail;
            gameId = gamesdata[i].gameId;
            gameName = gamesdata[i].gameName;
            collector.addGamesDesigned(email, gameId, gameName, dbProvider);
        }
        res.json("done");
    });
    ...
    router.get(collectorRoot + 'highscore_configure', function (req, res) {
       // console.log("in rest: "+JSON.parse(req.headers.jsonobject).email);
        processResponse(collector.highscore_configure(req.headers.jsonobject), res);
    });
    router.get(collectorRoot + 'experience_configure', function (req, res) {
       processResponse(collector.experience_configure(req.headers.jsonobject), res);
    });
