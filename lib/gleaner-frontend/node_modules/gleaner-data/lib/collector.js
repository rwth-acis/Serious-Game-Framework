module.exports = (function() {
    ...
    return {
        ...
        addGamesDesigned: function(email, gameId, gameName, dbProvider) {

            var profs = dbProvider.db().collection("profiles");
           // console.log(email + gameId + gameName);
            return profiles.find({email: email, "designed.gameId": gameId}, true).then(function(record) {
             if(record) {
                //console.log("record found");
            }
            else {
                    // If not, create with gameId
                    profs.update({email: email}, {$addToSet: {designed: { gameId: gameId, gameName: gameName}}}, function(err, result) {
                       // console.log("added");
                        // console.log("RESULT: " + result);
                    });
                }
    ...
    throw {
        ...
        // Get the highScore of the given user as well as 2 High Scores above and 2 High Scores below
        highscore_configure: function(jsonobject) {
            // MARKO find profile of user with email
            var email = JSON.parse(jsonobject).email;

            var highscoreData = JSON.parse(jsonobject).highscoredata;
                ...
                if(record) {
                    return authTokens.track(record.authToken).then(function(authToken) {
                            ...
                            if(records) {
                                ...
                                for(var i = 0; i < records.length; i++) {
                                    ...
                                    for(var j= 0; j < highscoreData.length; j++){
                                        ...
                                        if(highscoreData[j].gameId == records[i].target_game){

                                            if(!highScores[records[i].user]) {
                                                highScores[records[i].user] = 0;
                                            }
                                            var existingValue = parseFloat(highScores[records[i].user]).toFixed(2);
                                            var value = 0;

                                            if(records[i].result == "correct") {
                                                var correctValue = parseFloat(highscoreData[j].correct).toFixed(2);
                                                if(correctValue < 0){
                                                    value = existingValue - Math.abs(correctValue);
                                                }else{
                                                    correctValue = -correctValue;
                                                    value = existingValue - correctValue;
                                                }
                                            }
                                            else if(records[i].result == "wrong") {
                                              var wrongValue =  parseFloat(highscoreData[j].wrong).toFixed(2);
                                              if(wrongValue < 0){
                                                value = existingValue -  Math.abs(wrongValue);
                                            }else{
                                                wrongValue = -wrongValue;
                                                value = existingValue -  wrongValue;
                                            }
                                        }
                                        else if(records[i].result == "show_me") {
                                            var showValue =  parseFloat(highscoreData[j].showMe).toFixed(2);
                                            if(showValue < 0){
                                                value = existingValue -  Math.abs(showValue);
                                            }else{
                                                showValue = -showValue;
                                                value = existingValue - showValue;
                                            }
                                        }           
                                        else if(records[i].result == "try_again") {

                                            var tryValue = parseFloat(highscoreData[j].tryAgain).toFixed(2);
                                            if(tryValue < 0){
                                                value = existingValue - Math.abs(tryValue);
                                            }else{
                                                tryValue = -tryValue;
                                                value = existingValue - tryValue;
                                            }
                                        }
                                        else if(records[i].result == "hint") {
                                           var hintValue = parseFloat(highscoreData[j].hint).toFixed(2);
                                           if(hintValue < 0){
                                            value = existingValue - Math.abs(hintValue);
                                        }else{
                                            hintValue = -hintValue;
                                            value = existingValue - hintValue;
                                        }
                                    }
                                    highScores[records[i].user] = parseFloat(value).toFixed(2); 
                                }
                            }
                        }
                        ...
                        if(highScores[email]) {}
                                ...
                                var scores = [];
                                for (var player in highScores) {
                                    var roundedScore = parseFloat(highScores[player]).toFixed(0);
                                    scores.push({user: player, score: roundedScore});
                                }
...
experience_configure: function(jsonobject) {
    var email = JSON.parse(jsonobject).email;
    var highscorevalue = JSON.parse(jsonobject).highscorevalue;
    var experiencerules = JSON.parse(jsonobject).experiencerules;
    var experiencebadges = JSON.parse(jsonobject).experiencelevels;
    ...
    return profiles.find({email: email}, true).then(function(profile) {
        if(profile) {
            return authTokens.track(profile.authToken).then(function(authToken) {
                        ...
                        // 1. Add 150 points per created game
                        experience_points += profile.designed.length * experiencerules[0].gamesDesignedFactor;

                        // 2. Add 10 points per earned badge
                        profile.earnedBadges.forEach(function(badge) {
                            experience_points += badge.awarded * experiencerules[0].badgeFactor;
                        });
                        ...
                        traces_.find({user: email, target: "elearning"}).then(function(records) {
                            var points = 0;
                            if(records) {
                                points += records.length * experiencerules[0].eLearningLinkFactor;
                            }
                            experience_points += points;
                        });

                        traces_.find({user: email, target: "moreInformation"}).then(function(records) {
                            var points = 0;
                            if(records) {
                                points += records.length * experiencerules[0].moreInformationLinkFactor;
                            }
                            experience_points += points;
                        });

                        // 4. Add the highscore of this user
                        experience_points += highscorevalue * experiencerules[0].highscoreFactor;

                        // 5. Add the number of logins
                        return traces_.find({user: email, event: "login"}).then(function(records) {
                            if(records) {
                                experience_points += records.length * experiencerules[0].loginFactor;
                            }
                            ...
                            var level = -1;
                            var progress = 0;
                            for(var i= 0; i < experiencebadges.length; i++){
                               // console.log(experiencebadges[i])
                                if(experience_points >= experiencebadges[i].score){
                                    level++;
                                    if((i != experiencebadges.length - 1 && experience_points < experiencebadges[i+1].score) || i == experiencebadges.length - 1){
                                        experience_points -= experiencebadges[i].score;
                                        if(i != experiencebadges.length - 1){
                                            exp_to_next_level = experiencebadges[i+1].score - experience_points;
                                            progress = Math.round((experience_points / experiencebadges[i+1].score) * 100);
                                        }else{
                                            exp_to_next_level = "0 - Max Level";
                                            progress = 100;
                                        }
                                        level_name = experiencebadges[i].badgeDescription;
                                        index = i;
                                    }
                                }
