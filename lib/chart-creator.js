function ChartCreator() {
  ...
  this.changeChart = function(gameID, type, prefix) {
    ...
    $.ajax({
      url: "http://gaudi.informatik.rwth-aachen.de:3000/collect/traces/" + gameID, // TODO MARKO add real url
      dataType: "json",
      headers: {
  ...
  //New method of creating charts
    this.drawChartNew = function(stats, gameName, type, prefix) {
    var chart_type = type || 'pie';
    var prefix = prefix || 'player-';
    var chart_container = prefix + 'chart-container';
        ...
        for(var i = 0; i < stats.worst_levels.length; i++) {
         ...
         /* LEVELDATA[level_data.level].pieces.forEach(function(piece) {
          ...
          for(var j=1;j<=4;j++){
            ...
            if(GAMELEVELSSTATS.hasOwnProperty(level_data.level) && GAMELEVELSSTATS[level_data.level][id]  != ""){
               var tile = $('<li><img src="' + TILES_CONNECTIONS_PATH + GAMELEVELSSTATS[level_data.level][id] + '" ' +
              'title="' + GAMELEVELSSTATS[level_data.level][id] + '"></li>');
            $(worst_level_div).children('ul#level' + i).append(tile);
            }
          }
  ...
  this.changeChartNew = function(gameID, gameName, type, prefix) {
    // Query statistics for newly selected game
    $.ajax({
      url: "http://gaudi.informatik.rwth-aachen.de:3000/collect/traces/" + gameID, // TODO MARKO add real url
      dataType: "json",
      ...
      success: function(result) {
        // If everything went ok, draw the chart
        // TODO MARKO Worse then shit
        new ChartCreator().drawChartNew(result, gameName, type, prefix);
      },
      error: function(err) {
        console.log(err);
        console.log("Can't get traces. Server down?");
