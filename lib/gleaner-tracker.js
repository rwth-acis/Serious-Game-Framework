function GleanerTracker () {
  this.gleaner_path = "http://localhost:3000/"; // TODO MARKO use real location
  this.gleaner_url = this.gleaner_path + "collect/track";
  this.gleaner_traces_url = this.gleaner_path + "collect/traces";
  this.tracking_code = '56f6fced8476c17c194c5eb1x6st3nlzkuo9a4i'; // ENTER REAL TRACKINGCODE HERE
      ...
      if(result.statistic === "correct") {
        if(GAME_BADGES[i].badgeRequirementId == 2){
          if(result.count % GAME_BADGES[i].requirementValue === 0) {
            badge_asserter.assertBadgeConfigured("other", user_info, GAME_BADGES[i].badgeName,GAME_BADGES[i].badgeSrc);
          }
        } 
